...
# standardFilter <- function(t, x, y, settings, filterMarkerNames)
# {
...
#   {
#     stop("\r You must add 'screenResolution' element into the settings list of a data record!
#          \r It should be a vector with two numeric elements, e.g. c(1024,768).")
#   }
#   smpCnt <- length(t)
...
#   
#   gapFlags <- x == 0 & y == 0
#   outFlags <- x < 0 | x > screenResolution[1] | y < 0 | y > screenResolution[2]
...
#   
#   gapOutMarker <- paste(sort(c(gapMarker, outMarker)), collapse = "&")
#   
#   if (interpolateShort)
#   {
...
#       if (g$dur > minGapDuration & g$marker %in% c(gapMarker, gapOutMarker))
#       {
#         left <- g$prevPos; right <- g$nextPos
#         if (!is.na(left) & !is.na(right))
#         {
#           ###Left local minimum
#           while ((left - 1) > 0 & !(y[left] < y[left - 1] & y[left] < y[left + 1]))
#             left <- left - 1
#           ###Right local minimum
#           while ((right + 1) < smpCnt & !(y[right] < y[right + 1] & y[right] < y[right - 1]))
#             right <- right + 1
...
#   if (blinkDetection)
#   {
#     markers <- factor(df$marker, levels = c(okMarker, gapMarker, gapOutMarker, outMarker, bliMarker))
#   }
#   else
#   {
#     markers <- factor(df$marker, levels = c(okMarker, gapMarker, gapOutMarker, outMarker))
#   }
#   
#   groups <- markersGroups(markers)
#   res <- list(t = df$t, x = df$x, y = df$y, filterMarkers = list(eventMarkers = markers, 
#                                                                  eventGroups = groups,
#                                                                  eventsClass = "FilterEvents"))
#   return(res)
#   }
#Filter
standardFilter <- function(t, x, y, screenResolution = NULL, interpolateShort = T, blinkDetection = "zeros", 
  ...
  if (blinkDetection != "none") {
    ...
    if (blinkDetection == "peakY") {
      ...
      for(i in 1:nrow(gaps)) {
        ...
        if (g$dur > minGapDuration) {
          left <- g$prevPos; right <- g$nextPos
          #Left local minimun or gap start position
          if (is.na(left)) left <- g$startPos else
            while(left > 1 & !(y[left - 1] >= y[left] & y[left + 1] >= y[left]))
              left <- left - 1
            #Right local minimum or gap end position
...
dataFilter <- function(ETD, filterMarkerNames, filterSettings)
{
  ...
  filterSettings <- append(filterSettings, list(screenResolution = ETD$settings$screenResolution))
  if (mode == "left" | mode == "binocular")
  {
    filtResLeft <- standardFilter(t = ETD$commonData$time, x = ETD$leftEyeData$porx, y = ETD$leftEyeData$pory,
                                  settings = filterSettings, filterMarkerNames = filterMarkerNames) 
    if (interpolateShort)
    {
